...
class CUBDataset(Dataset):
    ...
    # def nearest_neighbors_resnet(self, k=3, power=1.5):
    #     preprocess = transforms.Compose([
    #         transforms.Resize(256),
    #         transforms.CenterCrop(224),
    #         transforms.ToTensor(),
    ...
    #         cosine(feature, centroid)
    #         for feature in features
    #     ])
    #
    #     distances_matrix = np.zeros((len(features), len(labeled_features)))
    #     for i in range(len(features)):
    #         for j in range(len(labeled_features)):
    #             cos_dist = cosine(features[i], labeled_features[j])
    #             centroid_dist = centroid_distances[i]
    #             distances_matrix[i, j] = (cos_dist ** power) / (centroid_dist + 1e-6)
    #
    #     indices = []
    #     weights = []
    #     for i in range(len(features)):
    #         k_nearest = np.argsort(distances_matrix[i])[:k]
    #         indices.append(k_nearest)
    #
    #         dist = distances_matrix[i][k_nearest]
