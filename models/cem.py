...
class ConceptEmbeddingModel(ConceptBottleneckModel):
    ...
    def _forward(
        ...
        with torch.no_grad():
            image_features = self.model_clip.encode_image(x)  # [batch, 3, 224, 224] -> [batch, 512]
    ...
    def plot_heatmap(
        ...
        #x = preprocess_2(x)
        #self.prs = hook_prs_logger(self.model_try, self.device)
        self.prs = hook_prs_logger(self.model_clip, self.device)
        self.prs.reinit()
        

        with torch.no_grad():
            image_features = self.model_clip.encode_image(x, attn_method='head', 
                                        normalize=False)  # [batch, 3, 224, 224] -> [batch, 512]
            attentions, mlps = self.prs.finalize(image_features)
        # image_features /= image_features.norm(dim=-1, keepdim=True)
