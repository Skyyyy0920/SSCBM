...
def visualize_and_save_attentionmaps(
    ...
    image = change_to_image(x)  # 转换成 (height, width, channels)
    ...
    for i in range(attention_map.shape[0]):      
        plt.figure(figsize=(6,6))                
        plt.imshow(image, alpha=1)
        attention_show = attention_map[i] - torch.mean(attention_map,axis=0)
        attention_show = (attention_show-torch.min(attention_show))/(torch.max(attention_show)-torch.min(attention_show))
        plt.imshow(attention_show, cmap='coolwarm', alpha=0.6)
        plt.colorbar()
        plt.axis('off')
        #plt.title(f'{concept_set[i]}    [{c_ground_truth[i]}] [{c_pred[i]:.3f}]')
